...
def train_ST_GATE(adata, hidden_dims=[512, 30], n_epochs=1000, lr=0.001, key_added='ST_GATE',
    ...
    model = Mix_ST_GATE(dim_input, dim_output, esparse=False, tau=tau, num_clusters=num_clusters,
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        loss_ll = model.ST_GATE.cont_ll(zs, zs_a, batch_size=batch_size)

        ret_cf = model.ST_GATE.cf_forward(zs, zs_a, cluster_temp=100)
        n_samples = zs.size(0)
        ...
        cont_cf = model.ST_GATE.cont_bxent(lbl, ret_cf)
    ...
    with torch.no_grad():
        ...
        if save_loss:
            adata.uns['ST_GATE_loss'] = loss.item() if not use_scheduler else best_loss
            adata.uns['ST_GATE_loss_history'] = loss_history

        if save_reconstrction:
            ReX = emb.to('cpu').detach().numpy()
            ReX[ReX < 0] = 0
            adata.layers['ST_GATE_ReX'] = ReX
